...
<template>
  <header class="w-full bg-white py-2 px-3 md:px-12 flex flex-row justify-between items-center border-b border-gray-200">
    ...
    <!-- Global Actions -->
    <div class="flex gap-5 md:gap-8 text-sm font-semibold text-gray-500 flex-shrink-0">

      <!-- Favorite/Wishlist Icon -->
      <div class="flex flex-col items-center justify-center cursor-pointer hover:text-red-500 transition">
        <svg class="w-5 h-5 md:w-7 md:h-7 mb-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
        </svg>
        <span class="text-[8px] md:text-[10px] tracking-wider">FAVORITE</span>
      </div>
      ...
      <div class="flex flex-col items-center justify-center cursor-pointer hover:text-blue-600 transition relative">
        ...
        <span v-if="cartItemCount > 0" class="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] text-white font-bold">
          {{ cartItemCount }}
        </span>
        <span class="text-[8px] md:text-[10px] tracking-wider">CART</span>
      </div>
      ...
      <div class="flex flex-col items-center justify-center cursor-pointer hover:text-blue-600 transition">
        ...
        <span class="text-[8px] md:text-[10px] tracking-wider">ACCOUNT</span>
      </div>
    </div>
  </header>
</template>
